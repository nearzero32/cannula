...
describe('Pharmacy atomic dispatch and quotation CAS', () => {
    test('available pool requires an operational Pharmacy and excludes its rejected requests', async () => {
        const service=new PharmacyTreatmentRequestService(directPharmacyTransactionRunner, undefined, noNotifications); spyOn(pharmacyService,'requireOperational').mockResolvedValue({_id:pharmacyA} as any);
    ...
    test('quote revision CASes dispatch and quote versions and computes totals', async () => {
        const service=new PharmacyTreatmentRequestService(directPharmacyTransactionRunner, undefined, noNotifications),current=request(S.WAITING_CUSTOMER_APPROVAL,pharmacyA,4,2),updated=request(S.WAITING_CUSTOMER_APPROVAL,pharmacyA,5,3);
    ...
    test('stale patient decision loses and exact rejection reopens with an exclusion', async () => {
        const service=new PharmacyTreatmentRequestService(directPharmacyTransactionRunner, undefined, noNotifications),current=request(S.WAITING_CUSTOMER_APPROVAL,pharmacyA,2,2); spyOn(TreatmentRequest,'findOne').mockReturnValue(query(current) as never); quiet();
    ...
    test('acceptance writes an immutable snapshot of the exact active quotation', async () => {
        const service=new PharmacyTreatmentRequestService(directPharmacyTransactionRunner, undefined, noNotifications),current=request(S.WAITING_CUSTOMER_APPROVAL,pharmacyA,5,3),updated=request(S.CONFIRMED,pharmacyA,6,3);
    ...
    test('critical history failure prevents the workflow operation from reporting success', async () => {
        const failing={create:async()=>{throw new Error('history unavailable')}}; const service=new PharmacyTreatmentRequestService(directPharmacyTransactionRunner,failing,noNotifications);
    ...
    test('excluded Pharmacy requires both explicit Admin override and a reason',async()=>{
        const current=request(S.OPEN,null,4,0);current.excluded_pharmacy_ids=[pharmacyA];const service=new PharmacyTreatmentRequestService(directPharmacyTransactionRunner, undefined, noNotifications);
    ...
    test('lifecycle is scoped to the current Pharmacy and cannot silently skip', async () => {
        const service=new PharmacyTreatmentRequestService(directPharmacyTransactionRunner, undefined, noNotifications); spyOn(pharmacyService,'requireOperational').mockResolvedValue({_id:pharmacyA} as any); quiet(); const update=spyOn(TreatmentRequest,'findOneAndUpdate');
...
describe('Pharmacy account and privacy boundaries', () => {
    test('request creation ActivityLog excludes prescription and delivery contents', async () => {
        const service=new PharmacyTreatmentRequestService(directPharmacyTransactionRunner, undefined, noNotifications),created={...request(S.OPEN,null,0,0),prescription_images:['private-key'],treatment_details:'sensitive treatment',delivery_address:{address_text:'private address',lat:33,lng:44},delivery_phone:'07700000000',notes:'private note'} as any;
    ...
    test('creation rejects empty content before persistence', async () => {
        const service=new PharmacyTreatmentRequestService(directPharmacyTransactionRunner, undefined, noNotifications); const create=spyOn(TreatmentRequest,'create');
