...
import { AppointmentActorTypeEnum, AppointmentBeneficiaryTypeEnum, IAppointmentBookingSourceEnum } from '../src/interfaces/appointment.interface';
...
describeWithMongo('Appointment concurrency against MongoDB replica set', () => {
    ...
    const fakeSlots = {
        async requireSlot(input: any, options: { session?: ClientSession | null } = {}) {
            ...
            return {
                ...
                context: { doctor: { _id: doctorId, display_name: 'طبيب اختبار', profile_photo: null, consultation_fee: 25000, currency: 'IQD', accept_auto_booking: false }, clinic: { _id: clinicId, name: 'عيادة اختبار', address: 'بغداد' }, specialty: null },
            };
        },
    };
    ...
    test('failed destination booking rolls back and leaves the original unchanged', async () => {
        ...
        await expect(service.reschedule(String(original._id), { date, startsAt: localDateTimeToUtc(date, '10:00').toISOString() }, actor)).rejects.toMatchObject({ code: 'APPOINTMENT_SLOT_CONFLICT' });
