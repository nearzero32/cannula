...
import { AppointmentActorTypeEnum, AppointmentAvailabilityStatusEnum, AppointmentBeneficiaryTypeEnum, IAppointmentBookingSourceEnum, IAppointmentPaymentStatusEnum, IAppointmentStatusEnum } from '../interfaces/appointment.interface';
...
export const AppointmentSchema = t.Object({
    ...
    dailyCapacity: t.Optional(t.Object({ max: t.Integer(), booked: t.Integer(), remaining: t.Integer(), reached: t.Boolean() })),
...
export const SlotSchema = t.Object({ startsAt: t.String({ format: 'date-time' }), endsAt: t.String({ format: 'date-time' }), localStartsAt: t.String(), localEndsAt: t.String() });
export const DailyCapacitySchema = t.Object({ max: t.Integer(), booked: t.Integer(), remaining: t.Integer(), reached: t.Boolean(), availableSlotCount: t.Integer(), bookableRemaining: t.Integer() });
export const NextAvailableSchema = t.Object({ date: t.String({ format: 'date' }), startsAt: t.String({ format: 'date-time' }), endsAt: t.String({ format: 'date-time' }), localStartsAt: t.String(), localEndsAt: t.String() });
export const SlotsResponseSchema = t.Object({ error: t.Literal(false), message: t.String(), data: t.Object({ doctorId: t.String(), clinicId: t.String(), date: t.String({ format: 'date' }), timezone: t.Literal('Asia/Baghdad'), availabilityStatus: t.Enum(AppointmentAvailabilityStatusEnum), dailyCapacity: DailyCapacitySchema, slots: t.Array(SlotSchema), nextAvailable: t.Nullable(NextAvailableSchema), nextAvailableOptions: t.Array(NextAvailableSchema), nextAvailabilityStatus: t.Optional(t.Enum(AppointmentAvailabilityStatusEnum)) }) });
...
export const SettingsBodySchema = t.Object({ appointment_duration: t.Optional(t.Integer({ minimum: 5, maximum: 480 })), slot_interval: t.Optional(t.Integer({ minimum: 5, maximum: 480 })), buffer_before: t.Optional(t.Integer({ minimum: 0, maximum: 240 })), buffer_after: t.Optional(t.Integer({ minimum: 0, maximum: 240 })), booking_lead_time_hours: t.Optional(t.Number({ minimum: 0, maximum: 8760 })), cancellation_window_hours: t.Optional(t.Number({ minimum: 0, maximum: 8760 })), max_appointments_per_day: t.Optional(t.Integer({ minimum: 1, maximum: 200 })), accept_auto_booking: t.Optional(t.Boolean()), allow_reschedule: t.Optional(t.Boolean()), accepting_new_patients: t.Optional(t.Boolean()) }, { additionalProperties: false, minProperties: 1 });
...
export const BookingSettingsSchema = t.Object({ appointmentDuration: t.Number(), slotInterval: t.Number(), bufferBefore: t.Number(), bufferAfter: t.Number(), bookingLeadTimeHours: t.Number(), cancellationWindowHours: t.Number(), maxAppointmentsPerDay: t.Number(), acceptAutoBooking: t.Boolean(), allowReschedule: t.Boolean(), acceptingNewPatients: t.Boolean() });
