...
function handle(error: unknown, set: { status?: number | string }) { if (!(error instanceof DomainError)) throw error; set.status = error.status; return { error: true as const, message: error.message }; }
...
export const authSecurityController = new Elysia({ prefix: '/auth-security', detail: { tags: [SWAGGER_TAGS.ADMIN.AUTH_SECURITY] } })
    ...
    .post('/flows/:flowId/otp-assistance', async ({ params, body, phrase, headers, set }) => {
        try { await requireAdminPermission(phrase.role, phrase._id, IAdminPermissionEnum.ISSUE_SUPPORT_OTP); return { error: false, message: 'تم إصدار رمز الدعم؛ سيظهر مرة واحدة فقط', data: await patientAuthService.issueSupportOtp(params.flowId, body.reason, phrase._id, ip(headers)) }; }
        catch (error) { return handle(error, set); }
    ...
    .post('/patients/:patientId/reset-pin', async ({ params, body, phrase, headers, set }) => {
        try { await requireAdminPermission(phrase.role, phrase._id, IAdminPermissionEnum.RESET_PATIENT_PIN); return { error: false, message: 'تم تعيين رمز مؤقت وإلغاء جميع الجلسات؛ سيظهر الرمز مرة واحدة فقط', data: await patientAuthService.adminResetPin(params.patientId, body.reason, phrase._id, ip(headers)) }; }
        catch (error) { return handle(error, set); }
    ...
    .post('/patients/:patientId/revoke-sessions', async ({ params, body, phrase, headers, set }) => {
        try { await requireAdminPermission(phrase.role, phrase._id, IAdminPermissionEnum.REVOKE_PATIENT_SESSIONS); return { error: false, message: 'تم إلغاء جلسات المريض بنجاح', data: await patientAuthService.revokePatientSessions(params.patientId, phrase._id, body.reason, ip(headers)) }; }
        catch (error) { return handle(error, set); }
