...
import {
    ...
    HomeCareWeeklyAvailabilityResponseSchema,
...
const servicesController = new Elysia({
    ...
    .get('/:id/availability', async ({ params, phrase, set }) => {
        ...
        return { error: false, message: 'تم جلب جدول التوفر الأسبوعي بنجاح', data: await homeCareAvailabilityService.listForDashboard(params.id) };
    }, { params: t.Object({ id: t.String() }), response: { 200: HomeCareWeeklyAvailabilityResponseSchema, 400: BadRequestResponseSchema, 404: NotFoundResponseSchema, ...protectedResponses } })
    .put('/:id/availability', async ({ params, body, phrase, set }) => {
        ...
        const schedule = await homeCareAvailabilityService.replaceWeekly(params.id, body, slotActor(phrase, `/dash/admin/home-care/services/${params.id}/availability`));
        return { error: false, message: 'تم حفظ جدول التوفر الأسبوعي بنجاح', data: schedule };
    }, {
        params: t.Object({ id: t.String() }),
        body: t.Object({ schedule: t.Array(t.Object({
            day_of_week: t.Enum(HomeCareWeekdayEnum),
            times: t.Array(t.String({ pattern: '^([01]\\d|2[0-3]):[0-5]\\d$' }), { maxItems: 24, uniqueItems: true }),
        }, { additionalProperties: false }), { minItems: 7, maxItems: 7 }) }, { additionalProperties: false }),
        response: { 200: HomeCareWeeklyAvailabilityResponseSchema, 400: BadRequestResponseSchema, 404: NotFoundResponseSchema, 409: ConflictResponseSchema, 422: ValidationErrorResponseSchema, ...protectedResponses },
    })
    .get('/:id', async ({ params, phrase, set }) => {
        if (!await hasAccess(phrase._id, phrase.role, 'read')) {
            set.status = 403;
