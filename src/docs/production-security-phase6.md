...
Swagger is enabled by default only outside production. `ENABLE_SWAGGER=true` in production additionally requires a random 32+ character `SWAGGER_ADMIN_TOKEN`, supplied as a Bearer token. Prefer leaving both UI and JSON disabled publicly.
