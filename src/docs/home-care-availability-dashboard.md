...
- `PATCH /dash/admin/home-care/services/:serviceId/availability/:slotId` edits time/order.
...
Requests snapshot both `availability_slot_id` and the selected slot's then-current `time` as `preferred_time`. Editing or disabling a slot never rewrites historical requests. Availability is not Redis-cached, so dashboard changes are visible immediately.
