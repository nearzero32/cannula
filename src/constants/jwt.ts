...
interface TokenIdentity { _id: string; role: IUserRole; sid: string; aud: TokenAudience; sub: string }
...
export function signAccessToken(data: { _id: string; role: IUserRole; sid: string; audience: TokenAudience }): string {
    return jwt.sign(
        { _id: data._id, role: data.role, sid: data.sid, tokenType: 'access' },
        process.env.ACCESS_TOKEN_SECRET!,
        { expiresIn: '15m', audience: data.audience, subject: data._id, jwtid: crypto.randomUUID() }
    );
}

export function signRefreshToken(data: { _id: string; role: IUserRole; sid: string; jti: string; audience: TokenAudience }): string {
    return jwt.sign(
        { _id: data._id, role: data.role, sid: data.sid, tokenType: 'refresh' },
        process.env.REFRESH_TOKEN_SECRET!,
        { expiresIn: '7d', audience: data.audience, subject: data._id, jwtid: data.jti }
    );
}
